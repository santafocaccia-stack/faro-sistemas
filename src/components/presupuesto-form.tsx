...
export function PresupuestoForm({ productos, clientes }: Props) {
  ...
  function agregarLineaLibre() {
    setLineas((prev) => [
      ...prev,
      { key: crypto.randomUUID(), productoId: null, descripcion: '', cantidad: 1, precioUnitario: 0 },
    ]);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        <h2 className="text-sm font-semibold tracking-tight">Agregar productos</h2>
        ...
        {busqueda && (
          <div className="rounded-lg border border-border bg-background/60 overflow-hidden max-h-52 overflow-y-auto">
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        {lineas.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">Buscá un producto para agregarlo</p>
          </div>
        ) : (
          <>
            {/* Encabezado tabla */}
            <div className="grid grid-cols-[1fr_80px_120px_100px_36px] gap-2 px-4 py-2 bg-muted/30 border-b border-border/60">
              {['Descripción', 'Cant.', 'P. unit.', 'Subtotal', ''].map((h) => (
                <p key={h} className="text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground/70">{h}</p>
              ))}
            </div>
            ...
            {lineas.map((l) => {
              ...
              return (
                <div
                  key={l.key}
                  className="grid grid-cols-[1fr_80px_120px_100px_36px] gap-2 px-4 py-2.5 border-b border-border/40 last:border-0 items-center"
                >
                  <Input
                    value={l.descripcion}
